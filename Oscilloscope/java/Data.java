...
class Data {
    ...
    int getData(int nodeId, int x, int dataType) {
        if (nodeId >= nodes.length || nodes[nodeId] == null)
            return -1;
        return nodes[nodeId].getData(x, dataType);
    }

    /* Return number of last known sample on mote nodeId. Returns 0 for
       unknown motes. */
    int maxTime(int nodeId) {
        if (nodeId >= nodes.length || nodes[nodeId] == null)
            return 0;
        return nodes[nodeId].maxTime();
    }
